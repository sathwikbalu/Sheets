  ...
  document.querySelectorAll('.cell[contenteditable="true"]').forEach((cell) => {
    ...
    cell.addEventListener("keypress", (e) => {
      if (e.key === "Enter") {
        ...
        const formulaBarValue = document
...
function parseRange(operation) {
  ...
  const startCol = start[0],
    startRow = parseInt(start.slice(1));
  const endCol = end[0],
    endRow = parseInt(end.slice(1));
...
function moveToNextCell() {
  const cells = Array.from(
    document.querySelectorAll('.cell[contenteditable="true"]')
  );
...
function saveSpreadsheet() {
  ...
  if (chartCanvas) {
    ...
    const chartImageSheet = XLSX.utils.aoa_to_sheet([
      ["Chart Data"],
      ["Image Below"],
    ]);
    ...
    alert(
      "Note: Excel doesn't natively embed images. You can reference the exported image."
    );
  }
...
function testFunctions() {
  ...
  const workbook = XLSX.utils.book_new();
  const testSheetData = testResults.map((test) => [
    test.formula,
    test.result,
    test.expected,
    test.status,
...
function addColumn() {
  ...
  rows.forEach((row, index) => {
    ...
    newCell.setAttribute(
      "data-cell",
      `${getColumnLabel(colCount)}${index + 1}`
    );
